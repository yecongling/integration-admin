...
export enum EndpointTypeApi {
  ...
  // 获取端点类型 行数据
  getEndpointType = "/project/endpointType/getEndpointType",
  // 查询端点类型配置项
 ...
 * @param title 查询条件
 */
export const getEndpointType = (title?: string) => {
  return HttpRequest.get<EndpointType[]>({ url: EndpointTypeApi.getEndpointType, params: { title } }, { successMessageMode: "none" })
}
